...
<template>
  <div class="max mt-80 h-full w-[100vw] bg-[#fff]">
    ...
    <div class="w-[100vw] text-center text-left">
      <div
        v-if="isPCDevice"
        class="relative h-300 w-1920 f-c flex-col text-align-left color-[#fff]"
      >
        <div class="bannerImgTransfrom absolute left-[50%] top-0 z-0 h-full w-full">
          <img
            src="@/assets/images/game_list/game_banner.png"
            alt=""
            h-full
            w-full
          >
      ...
      <div
        v-if="!isPCDevice"
        class="m-auto mt-43 w-full px-29 pb-20 pt-30"
        :class="{ 'min-w-1440 max-w-1560 px-40 pt-50 pb-39': isPCDevice }"
      >
        <div
          v-for="(gameCard, index) in gameCardList"
          :key="gameCard.title"
          class="gameCardShadow mb-30 h-57.6vw w-full flex cursor-pointer bg-[#eee]"
          :class="{ 'gameCardPC': isPCDevice, 'mr-60': index % 2 === 0 && isPCDevice }"
          :style="{ backgroundImage: `url('${gameCard.bgImage}')`, backgroundSize: '100%' }"
        >
          <div
          ...
          >
            <div
              ...
              :class="{ 'w-110 h-110': isPCDevice, 'h-[16vw] w-[16vw]': !isPCDevice }"
            >
              <img
                :src="gameCard.avaterImage"
                class="h-full w-full"
                alt=""
                :class="{ 'rounded-22': isPCDevice, 'rounded-3.2vw': !isPCDevice }"
              >
            </div>
            <div
              class="w-full break-all color-[#333]"
              :class="{ 'text-28 leading-34 mt-20 mb-6': isPCDevice, 'mb-4 mt-12 leading-23 text-26': !isPCDevice }"
            >
              {{ gameCard.title }}
            </div>
            <div
              class="mt-20 pr-10 color-[#666]"
              :class="{ 'text-14 leading-17': isPCDevice, 'leading-15 text-18': !isPCDevice }"
            >
              {{ gameCard.description }}
            </div>
            <div
              v-if="isPCDevice"
              class="gameCardExplore"
            >
              探索
            </div>
          </div>
          <div
            class="relative inline-block text-right align-top"
            :class="{ 'w-420 h-420': isPCDevice, ' w-57.333vw h-full': !isPCDevice }"
            :style="{ backgroundImage: `url('${gameCard.bgImage}')`, backgroundSize: '100%' }"
          >
            <img
              :src="gameCard.logoImage"
              alt=""
              class="object-contain"
              :class="{ 'w-165 h-90 mt-330': isPCDevice, 'mt-45.333vw h-46 w-84': !isPCDevice }"
            >
          </div>
        </div>
      </div>
      <div
        v-if="isPCDevice"
        class="relative mb-40 ml-74 mr-58 mt-77"
      >
